...
def build_refinement_prompt(query, schema, user_question, error_message=""):
    """
...
1. **Validation Against Schema:** Validate that the refined query uses only the columns and tables provided in the schema: {schema_description}. Ensure there are no references to non-existent or incorrectly named tables/columns only take columns and tables present in this schema {schema_description}.
2. **Error Correction:** Correct any syntax errors in the SQL query: {query}. Make sure the structure of the query follows SQL standards and best practices.
3. **Query Optimization:** Optimize the query for performance by reducing unnecessary joins, using indexed columns, and minimizing data retrieval.
4. **Specific Column Selection:** Avoid using `SELECT *`. Always specify the exact columns needed to answer the user's question based on the schema.
5. **Handling Large Datasets:** If the user's question does not specify a limit, include a `LIMIT 100` clause to manage large datasets efficiently.
6. **Alignment with User Intent:** Ensure that the final query generated by you accurately reflects the user's intent {user_question} as described in the original question. Adjust the query logic if needed to better meet the user's requirements.
...
Instructions:
1. Correct the SQL query based on the schema {schema_description}. Ensure all tables and columns are valid and double check the syntax by referring to  this {schema_description} only.
...
def iterative_refinement(query, schema, error_message="" ,flag=0):
    ...
    while iterations < max_iterations:
        ...
        if flag == 1 :
            ...
            prompt = build_refinement_prompt(refined_query, schema, user_training_question, error_message)
